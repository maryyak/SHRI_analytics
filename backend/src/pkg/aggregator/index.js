...
const parseNumberFromBuffer = (buffer, start, end) => {
  ...
  for (let i = start; i < end; i++) {
    ...
    const isDigit = currentCharCode >= CharCodes.ZERO && currentCharCode <= CharCodes.NINE;
...
export const aggregate = (params) => {
  ...
  civHashToString.set(hashBuffer(Buffers.HUMAN, 0, Buffers.HUMAN.length), 'humans');
  civHashToString.set(hashBuffer(Buffers.BLOBS, 0, Buffers.BLOBS.length), 'blobs');
  civHashToString.set(hashBuffer(Buffers.MONSTERS, 0, Buffers.MONSTERS.length), 'monsters');
  ...
  const processHeaders = (lineStart, lineEnd) => {
    ...
    const processField = (fieldEnd) => {
      const compare = compareBuffers.bind(null, buffer, fieldStart, fieldEnd - fieldStart);
